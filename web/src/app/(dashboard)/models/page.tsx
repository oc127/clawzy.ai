...
const TIER_CONFIG: Record<string, { label: string; bg: string; text: string; gradient: string }> = {
  premium: { label: "Premium", bg: "bg-amber-100 dark:bg-amber-900/30", text: "text-amber-700 dark:text-amber-400", gradient: "icon-gradient-orange" },
  standard: { label: "Standard", bg: "bg-blue-100 dark:bg-blue-900/30", text: "text-blue-700 dark:text-blue-400", gradient: "icon-gradient-blue" },
  free: { label: "Free", bg: "bg-emerald-100 dark:bg-emerald-900/30", text: "text-emerald-700 dark:text-emerald-400", gradient: "icon-gradient-green" },
};
...
export default function ModelsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-extrabold text-[#222222] dark:text-white">Models</h1>
          <p className="mt-0.5 text-[#717171] dark:text-[#a0a0a0]">Available AI models for your agents.</p>
        </div>
        <Link href="/agents">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {models.map((model) => {
          ...
          return (
            <div
              key={model.id}
              className="flex flex-col rounded-2xl border border-[#ebebeb] dark:border-[#333] bg-white dark:bg-[#1a1a1a] p-5 shadow-[0_2px_8px_rgba(0,0,0,0.06)] hover:shadow-[0_6px_20px_rgba(0,0,0,0.10)] hover:-translate-y-0.5 transition-all duration-200"
            >
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <h3 className="font-bold text-[#222222] dark:text-white truncate">{model.name}</h3>
                  <div className="mt-1 flex flex-wrap gap-1.5">
                    <span className="rounded-full bg-[#f7f7f7] dark:bg-[#262626] border border-[#ebebeb] dark:border-[#333] px-2.5 py-0.5 text-xs font-medium text-[#717171] dark:text-[#a0a0a0]">
                      {model.provider}
                    </span>
              ...
              <p className="mt-3 text-sm text-[#717171] dark:text-[#a0a0a0] leading-relaxed flex-1">{model.description}</p>
              <div className="mt-4 flex justify-between rounded-xl bg-[#f7f7f7] dark:bg-[#262626] px-4 py-2.5 text-xs text-[#717171] dark:text-[#a0a0a0]">
                <span>{model.credits_per_1k_input} cr / 1K in</span>
                <span className="text-[#dddddd] dark:text-[#444]">·</span>
      ...
      {models.length === 0 && (
        <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-[#dddddd] dark:border-[#444] bg-white dark:bg-[#1a1a1a] py-16 text-center">
          <Cpu className="mb-3 h-10 w-10 text-[#b0b0b0] dark:text-[#666]" />
          <p className="text-[#717171] dark:text-[#a0a0a0]">No models available yet.</p>
        </div>
      )}
    </div>
  );
